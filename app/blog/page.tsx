...
function SectionDivider({ label, isQuant }: { label: string; isQuant: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.7rem", color: isQuant ? "#64FFDA" : "#7B9EFF", letterSpacing: "0.08em" }}>
        {label}
      </span>
    ...
    posts.map((post) => (
      <Link key={post.slug} href={`/blog/${post.slug}`} id={`blog-${post.slug}`} style={{ textDecoration: "none" }}>
        <article className="card" style={{ padding: "24px", display: "flex", gap: "20px", alignItems: "flex-start" }}>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: "1rem", color: "#E2E8F0", marginBottom: "8px", lineHeight: 1.4 }}>
              {post.title}
            </h2>
            <p style={{ color: "#64748B", fontSize: "0.85rem", lineHeight: 1.6, marginBottom: "12px" }}>{post.excerpt}</p>
            <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center" }}>
              <span style={{ color: "#475569", fontSize: "0.72rem", fontFamily: "'JetBrains Mono', monospace" }}>
                {formatDate(post.date)}
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: "4px", color: "#475569", fontSize: "0.72rem", fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          <ArrowRight size={16} color="#64748B" style={{ flexShrink: 0, marginTop: "4px" }} />
        </article>
      </Link>
    ));
      ...
      <section style={{ padding: "80px 0" }}>
        <div className="section-container">
          <div style={{ marginBottom: "48px" }}>
            ...
            <p style={{ color: "#94A3B8", fontSize: "0.95rem", lineHeight: 1.7, maxWidth: "540px", marginTop: "16px" }}>
